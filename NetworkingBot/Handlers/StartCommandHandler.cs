...
internal class StartCommandHandler(ILogger<StartCommandHandler> logger, IUserStorage userStorage) : ISlashCommandHandler<StartCommand>
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await userStorage.WithCreateOrGetUser(chat, user, async _ =>
        {
            await botClient.SendMessage(chat.Id, Texts.Welcome(Commands.Commands.Join, Commands.Commands.Postpone),
                replyMarkup: new InlineKeyboardMarkup(Commands.Commands.Join.Button(chat.Id), Commands.Commands.Postpone.Button(chat.Id)), 
